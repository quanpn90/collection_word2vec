...
import zipfile
from model import ProximitySkipGram, UnigramLanguageModel
...
def extract_skip_grams(root, text_sources, word2idx, unigram_model, skip_window=3, shuffling=True):
	...
	for c, text_source in enumerate(text_sources):
		...
		for i, word in enumerate(words):
			...
			for context in contexts:
				...
				data.append([pos_input, 1])
				data.append([neg_input, 0])
	...
	for p_n in data:
		train_inputs.append(p_n[0])
		train_labels.append(p_n[1])

	train_tensors = np.asarray(train_inputs, dtype=np.int32)
	label_tensors = np.asarray(train_labels, dtype=np.int32)

	return train_tensors, label_tensors
...
def main():
	...
	input_tensors, label_tensors = extract_skip_grams(root, text_sources, word2idx, unigram_model) 
	...
	batch_x = T.imatrix() # Change to imatrix if input has more than 1 word 
	y = T.ivector()
	valid = T.ivector()
	x = T.ivector() # non-batch tensor input
	index = T.lscalar()
	model = ProximitySkipGram(rng, batch_x, vocab_size, embedding_size, hidden_size=hidden_size) 
	...
	train_model = theano.function(inputs=[index], # batch index
	                              outputs=loss,
	                              updates=updates,
	                              givens={batch_x: shared_x[batch_interval],
	                                      y: shared_y[batch_interval]}
	                              )
	...
	for iter in xrange(n_iter):
		...
		for index in xrange(num_steps):
			...
			if index % 1e6  == 0 or index == num_steps - 1:
